...
class SlidingWindow:
    ...
    def slidingwindow(self, img):
		...
		#good_left = ((nonzero_x >= 10) & (nonzero_y >= 380) & (nonzero_x <= width/2 - 80)).nonzero()[0]
                good_left = ((nonzero_x >= 10) & (nonzero_y >= 300) & (nonzero_x <= width/2 - 120)).nonzero()[0]
		#good_right = ((nonzero_x >= width/2 + 80) & (nonzero_y >= 380) & (nonzero_x <= width-10)).nonzero()[0]
                good_right = ((nonzero_x >= width/2 + 160) & (nonzero_y >= 380) & (nonzero_x <= width-60)).nonzero()[0]

		# draw left square
		square_left = np.array([[10, height], [10, 380], [200, 380], [200, height]], np.int32)
		cv2.polylines(out_img, [square_left], False, (0,255,0), 1)
		# draw right square
		square_right = np.array([[width/2 + 80, height], [width/2 + 80, height - 80], [width-60, height - 110], [width-60, height]], np.int32)
		cv2.polylines(out_img, [square_right], False, (255,0,0), 1)
		# draw (0.340)
		draw_line = np.array([[0, 360], [width, 360]], np.int32)
		...
		for window in range(0, window_num):
		    ...
		    if flag == 1:
		        ...
		        if win_y_low >= 358 and win_y_low < 364:
		            x_location = x_represent + 175 #0.2
		    else: # right lane
		        win_y_low = y_represent - (window + 1) * window_height
		        win_y_high = y_represent - (window) * window_height
		        win_x_low = x_represent - margin
		        ...
		        if win_y_low >= 358 and win_y_low < 364:
		            x_location = x_represent - 180 #int(width*0.2) #0.2
