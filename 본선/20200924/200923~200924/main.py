...
def drive(Angle, Speed):
    ...
    if PART == 1:
        if Angle > 0.15 and Angle < -0.15 and car_run_speed >= 0.6:
	...
	car_run_speed = 0.4
        ...
        if car_run_speed <= 0.6:
            car_run_speed += 0.005 * 1.5

#	if Angle > 0.15 and Angle < -0.15 and car_run_speed <= 0.4:
 #           car_run_speed += 0.005 * 20
#	elif car_run_speed >= 0.6:
#	    car_run_speed = 0.6


#    if curve_count<2 or PART == 4:
#        if Angle > 0.15 and Angle < -0.15 and car_run_speed >= 0.6:
#            car_run_speed -= 0.005 * 8
#        elif car_run_speed <= 0.8:
#            car_run_speed += 0.005 * 1.5 #0.005 * 2
...
def start():
    ...
    out = cv2.VideoWriter(
        '/home/nvidia/Videos/9-23/original_{}-{}-{} {}-{}.avi'.format(now.year, now.month, now.day, now.hour,
                                                                      now.minute), cv2.VideoWriter_fourcc(*'MJPG'), 30,
        (640, 480))

    # sliding_window out_img video writer
    out2 = cv2.VideoWriter(
        '/home/nvidia/Videos/9-23/sliding_{}-{}-{} {}-{}.avi'.format(now.year, now.month, now.day, now.hour,
                                                                     now.minute), cv2.VideoWriter_fourcc(*'MJPG'), 30,
        (800, 448))
	...
	# LEFT VERSION
	    ...
	    #print('obstacle detected!')
            if (ob.mode == Position.LEFT):
                obstacle_cnt += 1
		if obstacle_cnt == 2:
		    for theta in range(270, 520, 19):   # (270, 515, 16)
                        st = 0.6 * np.sin(theta * np.pi / 180) #0.68
                        drive(st, car_run_speed)
                        time.sleep(0.1) 
		else:   
                    for theta in range(270, 530, 10):   # (270, 515, 16)
                        st = 0.71 * np.sin(theta * np.pi / 180) #0.68
                        drive(st, car_run_speed)
                        time.sleep(0.1)
                continue
                ...
                obstacle_cnt += 1
                for theta in range(280, 465, 10): # (280, 440, 10) #270,450, 8
	...
	cw_list.append(slidingwindow.cw_sum)
       ...
       # if slidingwindow.cw_sum >= 2900 and cw_sum_old >= 2900: # check!!!!!!!!!!!!
        if PART == 3:
	    #print('detected!')
            if sum(cw_list) // 4 >= 210:  # if stop_detected:
		print("crosswalk!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                drive(0, 0)
                time.sleep(5)
                crosswalk_cnt += 1
