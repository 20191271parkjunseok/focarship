...
def region_of_interest(image, width, height1, height2, roiw, roih):
    src = np.array([[width / 2 - 280, height1], [width / 2 + 280, height1], [width / 2 + 330, height2],
                    [width / 2 - 330, height2]], np.float32)
...
def slidingwindow(img):
    ...
    square_right = np.array(
        [[width / 2 + 20, height], [width / 2 + 20, height - 80], [width, height - 110], [width, height]], np.int32)
    ...
    for window in range(0, window_num):
        ...
        if flag == 1:
            ...
            cv2.rectangle(out_img, (win_x_low + int(width * 0.54), win_y_low),
                          (win_x_high + int(width * 0.54), win_y_high), (255, 0, 0), 1)

            good_left = ((nonzero_y >= win_y_low) & (nonzero_y < win_y_high) & (nonzero_x >= win_x_low) & (
            ...
            win_x_high = x_represent + margin
            cv2.rectangle(out_img, (win_x_low - int(width * 0.54), win_y_low),
                          (win_x_high - int(width * 0.54), win_y_high), (0, 255, 0), 1)
            cv2.rectangle(out_img, (win_x_low, win_y_low), (win_x_high, win_y_high), (255, 0, 0), 1)

            good_right = ((nonzero_y >= win_y_low) & (nonzero_y < win_y_high) & (nonzero_x >= win_x_low) & (
                        nonzero_x < win_x_high)).nonzero()[0]
...
def start():
    ...
    while True:
        ...
        detected = crosswalk_counter.check_crosswalk(image)
        ...
        # if stop_detected:  # stop line detected
        #     MODE = 0
        #     obstacle_count = 0
