...
class SlidingWindow:
    ...
    def slidingwindow(self, img):
		...
		if len(good_left) > 10:
		    flag = 1
		    x_represent_l = np.int(np.mean(nonzero_x[good_left]))
		    y_represent_l = np.int(np.mean(nonzero_y[good_left]))
		if len(good_right) > 10:
		    flag = 2
		    x_represent_r = np.int(np.mean(nonzero_x[good_right]))
		    y_represent_r = np.int(np.mean(nonzero_y[good_right]))
                else :
                    return out_img, None

		if abs(x_represent_l - x_represent_r) < 200:
		    x_represent = x_represent_r
		    y_represent = y_represent_r
		else:
		    x_represent = x_represent_l
		    y_represent = y_represent_l
