...
def process_image(frame):
    ...
    #low_threshold = 45
    #high_threshold = 100
    low_threshold = 60#5 #41
    high_threshold = 70#60 #70
...
def start():
    ...
    cap = cv2.VideoCapture("test_video/curve.avi")
   ...
   # crosswalk_counter = Crosswalk_Counter()
	...
	print('cw_sum_old', cw_sum_old)
        ...
        warp = region_of_interest(edge_img, 640, 390, 420, 640, 480)


        out_img, x_location = slidingwindow.slidingwindow(warp)

        if x_location is None:
            cv2.circle(out_img, (x_old, 400), 5, (255,0,0),-1)
            x_location = x_old
            #x_location = -100
